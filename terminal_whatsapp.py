...
lol = ['okay','alright','acha','fine','ok','Ok','achha']
cheat = ['send','send krde','krde send']
		...
		if x in chat.keys():
			...
			for i in range(len(cmd)):
				...
				if 'wiki' in x:
					...
					import wikipedia
					super_log(wikipedia.summary(topic,sentences=2) + '\n')
				...
				# 	playsound.playsound(music_dir_loc + "\\" + songs_list[int(r)])
					...
					super_log(random.choice(['G sir ? ','yes sir ? ']) + '\n')
					...
					super_log('I am JARVIS. your virtual ASSISTANT.' + '\n')
				# elif x.lower() in greeting_close:
				# 	log(random.choice(greeting_close) + 'sir !' + '\n')
				# 	os.system('cls')
				#	break
				elif x =='reboot ':
					super_log('Rebooting the Jarvis-Shell-Interface !')
				...
				# 	print(motivational_quotes[g])
					...
					try:
						...
						topic_file.close()
						super_log('File saved as ' + topic + '.txt')
					...
					super_log('current time is ' + str(time) + '\n')
				
				elif 'shutdown -h now ' in x :
					os.system('shutdown -s -t 0')
					super_log('shutting down the system ..')
					...
					pyperclip.copy(random.choice(lol))
					pt.click(543, 706)
					time.sleep(0.3)
					pt.hotkey('ctrl','v')
					time.sleep(0.3)
					pt.typewrite(['enter'])
					pt.click(213,451)				# random click to remove green dot
				elif x in cheat:
					pyperclip.copy('ha ha wait ...')
					pt.click(543, 706)
					time.sleep(0.3)
					pt.hotkey('ctrl','v')
					time.sleep(0.3)
...
while True:
		...
		if new_msg_loc ==None:
			...
			pt.moveTo(890,642)	
			time.sleep(0.5)					# message_box_loc = 543, 706
			pt.tripleClick(890,642)			
			time.sleep(2)
			pt.hotkey('ctrl','c')
			time.sleep(0.5)
			x = pyperclip.paste()
			...
			main()
				...
				pyperclip.copy('Unable to process the query..')
				pt.click(543, 706)
				time.sleep(0.3)
				pt.hotkey('ctrl','v')
				time.sleep(0.3)
